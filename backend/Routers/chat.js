...
router.post("/", async (req, res) => {
  try {
    ...
    const dedupeKey = `${sessionId}:${String(message || "").trim()}`;
    if (inFlightChatRequests.has(dedupeKey)) {
      const cachedReply = await inFlightChatRequests.get(dedupeKey);
...
`.trim();
    ...
    inFlightChatRequests.set(dedupeKey, aiPromise);
    try {
      reply = await aiPromise;
    } finally {
